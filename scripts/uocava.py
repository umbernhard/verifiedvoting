...
def make_map(fips):
    ...
    with open("../uocava_2012.svg", "w") as output:
        s = str(soup.prettify())
        s = s.replace("</defs>", "")
...
with open("../data/uocava_2012.csv") as f:
    state_swing = {}
    ...
    for row in csv.reader(f):
        ...
        if state == "ME":
            if row[0] != "2300000000":
                fips[row[0][0:5]] = {"score":0,"swing":state_swing[state]["party"], 
        ...
        if turnout[row[0]] > 0:
            score = absent/turnout[row[0]]
        else:
            score = 0

        if absent/turn > 1:
            print absent, turn
        if absent/turn > max_rate:
            max_rate = absent/turn
            print max_rate 

        fips[row[0][0:5]] = {"score":score,"swing":state_swing[state]["party"], 
                                                    "prob":state_swing[state]["prob"]} 
