        ...
        if p['id'] not in ["State_Lines", "separator"]:
            ...
            try:
                ...
                if p['id'][0:2] == "02":
                    ...
                    poll = int(fips_to_dre[p['id']]["poll"])
                    absentee = int(fips_to_dre[p['id']]["absentee"])

                    absentee_12 = int(fips_to_dre[p['id']]["2012_absentee"])
            ...
            if (((rate > 0) and poll == 1) or 
                    ((rate > 0) and (absentee > .8 or absentee_12 > .8)) or
                            (poll == 1 and (absentee > .8 or absentee_12 > .8))) and opacity < .55: 
...
#                if mail:
#                    pattern = "url(#mail);"
            elif (rate > 0 or poll == 1 or absentee > .8 or absentee_12 > .8) and opacity < .50:
 #               stroke = stroke + "stroke:#FFFF00;stroke-width:1;" 
...
#                print "vvpats: ", vvpats[code["fips_code"]]
    ...
    for row in csv.reader(open("../data/absentee.csv")):
        ...
        if row[0][0:5] in fips_to_dre.keys():
            fips_to_dre[row[0][0:5]]["absentee"]  = absent/turn
            fips_to_dre[row[0][0:5]]["2012_absentee"]  = 0
    ...
    for row in csv.reader(open("../data/pollworker.csv")):
        if row[0][0:5] in fips_to_dre.keys():
            fips_to_dre[row[0][0:5]]["poll"] = row[1][0]
